...
import { TTextareaAutoCapitalize, TTextareaWrap } from './mfp-textarea.types';
...
@Component({
  tag: 'mfp-textarea',
  styleUrl: './scss/mfp-textarea.scss',
  shadow: {
    delegatesFocus: true,
  },
})
export class MfpTextarea {
  // Own Properties
  // ====================
  ...
  @Element() el!: HTMLMfpTextareaElement;
  ...
  /**
   * The amount of time, in milliseconds, to wait before emitting the `mfpInput` event after the textarea value changes.
  ...
  @Event() mfpBlur!: EventEmitter<HTMLMfpTextareaElement>;
   ...
   */
  @Event() mfpChange!: EventEmitter<{ value: string; el: HTMLMfpTextareaElement }>;

  /** Callback handler emitted when the textarea value has been cleared */
  @Event() mfpClear!: EventEmitter<HTMLMfpTextareaElement>;

  /** Callback handler emitted when the textarea has received focus */
  @Event() mfpFocus!: EventEmitter<HTMLMfpTextareaElement>;
   ...
   */
  @Event() mfpInput!: EventEmitter<{ value: string; el: HTMLMfpTextareaElement }>;
  ...
  render() {
    return (
      <div class="mfp-textarea flex flex-auto flex-col" part="base">
        <label
          class={{ 'mfp-textarea__label': true, '!hidden': !this.hasLabel }}
        ...
        <div
          class={{
            'mfp-textarea__helper flex items-center justify-between': true,
            [`validation-${this.validationStatus}`]: true,
        ...
        >
          <span
            class="mfp-textarea__helper--text"
            ref={(span: HTMLElement) => (this.helperTextElem = span)}
            part="helper-text"
          >
            <slot name="helper-text" onSlotchange={this.handleHelperTextSlotChange} />
          </span>
          <span class={{ 'mfp-textarea__helper--counter': true, '!hidden': !this.maxlength }} part="helper-counter">

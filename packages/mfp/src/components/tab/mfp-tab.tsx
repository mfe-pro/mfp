...
import { TAB_ORIENTATION, TAB_PLACEMENT, TAB_SIZE, TTabOrientation, TTabPlacement, TTabSize } from './mfp-tab.types';
...
export class MfpTab {
  ...
  @Event() mfpClick: EventEmitter<HTMLMfpTabElement>;

  /** Handler to be called when the tab gets focus */
  @Event() mfpFocus: EventEmitter<HTMLMfpTabElement>;

  /** Handler to be called when the tab loses focus */
  @Event() mfpBlur: EventEmitter<HTMLMfpTabElement>;
   ...
   * This method is used inside `<mfp-tab-group>` to make tab focusable after the active one is focused
   */
  @Method()
  ...
  private get tabindex(): string {
    // NOTE: this.active is undefined when is not part of mfp-tab-group
  ...
  render() {
    return (
      <button
        ...
        class={{
          [`mfp-tab mfp-tab--${this.size} mfp-tab--${this.orientation}-${this.placement}`]: true,
      ...
      >
        <div class="flex items-center justify-center" part="content">
          ...
          <div class={{ 'line-clamp-1': true, 'ms-[--mfp-tab--label-icon-gap]': this.hasIcon }} part="text">
            <slot />
          </div>
        </div>
      </button>

...
@Component({
  tag: 'mfp-divider',
  styleUrl: './scss/mfp-divider.scss',
  shadow: true,
})
export class MfpDivider {
  // Own Properties
  // ====================
  ...
  @Element() el!: HTMLMfpDividerElement;
  ...
  render() {
    const styles = {
      ...(this.strokeColor && { '--mfp-divider--stroke-color': getColorCSSVariable(this.strokeColor) }),
      ...(this.strokeThickness && { '--mfp-divider--stroke-thickness': `${this.strokeThickness}px` }),
      ...(!isNil(this.strokeBasis) && { '--mfp-divider--stroke-basis': `${this.strokeBasis}px` }),
    };
    ...
    return (
      <Host style={styles}>
        <div
          class={{
            'mfp-divider': true,
            [`mfp-divider--${this.orientation}`]: true,
            [`mfp-divider--title__${this.titleAlignment}`]: true,
            'gap-0': !this.hasTitle,
          }}
          part="base"

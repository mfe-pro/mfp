...
import { SPINNER_SIZE, SPINNER_TEXT_POSITION, TSpinnerSize, TSpinnerTextPosition } from './mfp-spinner.types';
import {
  getCSSVariableValue,
  getTextContent,
  hasSlotContent,
...
@Component({
  tag: 'mfp-spinner',
  styleUrl: './scss/mfp-spinner.scss',
  shadow: true,
})
export class MfpSpinner {
  // Own Properties
  // ====================
  ...
  @Element() el!: HTMLMfpSpinnerElement;
  ...
  private setIconSize(): void {
    if (!this.hasIconSlot || !this.mfpIcon) return;

    this.mfpIcon.size = parseInt(getCSSVariableValue(`mfp-spinner--size-${this.size}`, this.el)).toString();
  }

  private get mfpIcon(): HTMLMfpIconElement | null {
    ...
    return [...slot.assignedElements({ flatten: true })].filter((el: Element) =>
      isHTMLElement(el, 'mfp-icon'),
    )[0] as HTMLMfpIconElement;
  }
  ...
  render() {
    return (
      <div
        class={{
          [`mfp-spinner ${this.size} text-${this.textPosition}`]: true,
      ...
      >
        {!this.hasIconSlot && (
          <div
            class={`mfp-spinner--loader ${this.size} relative text-[--mfp-spinner--color]`}
            aria-label="Loading..."
            role="status"
          >
        ...
        <span
          class={{
            'mfp-spinner--text font-medium leading-regular text-text-primary': true,

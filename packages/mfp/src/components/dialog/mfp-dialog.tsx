...
@Component({
  tag: 'mfp-dialog',
  styleUrl: './scss/mfp-dialog.scss',
  shadow: true,
})
export class MfpDialog {
  // Own Properties
  // ====================
  ...
  private OPEN_CSS_CLASS = 'mfp-dialog--open';

  // Reference to host HTML element
  // ===================================
  @Element() el!: HTMLMfpDialogElement;
  ...
  private handleClose = async () => {
    ...
    // Emit mfpAfterClose event after the dialog is closed
  ...
  private handleOpen = async () => {
    ...
    // Emit mfpAfterOpen event after the dialog is opened
    this.handleTransitionEnd();
  };
  ...
  render() {
    const style = {
      ...(this.border && { '--mfp-dialog--border-radius': `var(--mfp-radius--${this.border})` }),
    };
    ...
    return (
      <dialog
        style={style}
        class={{
          [`mfp-dialog hidden ${this.size} m-auto focus-visible:outline-none`]: true,
        ...
        aria-modal="true"
        aria-labelledby="mfp-dialog--title"
        part="dialog"
      >
        <main class="flex flex-col gap-[--mfp-dialog--title-body-gap] overflow-hidden" part="content">
          <header class="mfp-dialog--header" part="header">
            <div id="mfp-dialog--title" class="mfp-dialog--title flex flex-1 items-center justify-between" part="title">
              <slot name="title" />
            </div>
            <slot name="button-close">
              {!this.hideCloseButton && (
                <mfp-button class="mfp-dialog--close" appearance="text" size="small">
                  <mfp-icon class="cursor-pointer" name="x" role="img" title="Close" />
                </mfp-button>
              )}
            </slot>
          </header>
          <div
            class={{
              ...
              'overflow-y-auto p-i-[--mfp-dialog--padding]': this.hasContent,
              '!p-be-[--mfp-dialog--padding]': !this.hasFooter,
            }}
        ...
        <footer
          class={{
            ...
            'mfp-dialog--footer': this.hasFooter,

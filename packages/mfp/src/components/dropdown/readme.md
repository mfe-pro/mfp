...
| --------- | -------------------------------------------------------------------------- | --------------------------------- |
| `mfpOpen` | Callback handler to be called when the dropdown panel is opened or closed. | `CustomEvent<{ open: boolean; }>` |
...
| `"dropdown"` | The `<mfp-panel>` element used under the hood to display the dropdown panel              |
| `"panel"`    | The `<div>` element used to display and style the panel inside the `<mfp-panel>` element |
...
graph TD;
  ...
  style mfp-dropdown fill:#f9f,stroke:#333,stroke-width:4px
```

 ...
 * @part dropdown - The `<mfp-panel>` element used under the hood to display the dropdown panel
 * @part panel - The `<div>` element used to display and style the panel inside the `<mfp-panel>` element
 * @part trigger - The `<div>` element that hosts the trigger element
 */
@Component({
  tag: 'mfp-dropdown',
  styleUrl: './scss/mfp-dropdown.scss',
  shadow: true,
})
export class MfpDropdown {
  // Own Properties
  // ====================

  private dropdownPanelId = `mfp-dropdown-panel-${++id}`;
  private triggerElem: HTMLElement;

  // Reference to host HTML element
  // ===================================

  @Element() el!: HTMLMfpDropdownElement;
  ...
  @Watch('open')
  onOpenChange() {
    this.mfpOpen.emit({ open: this.open });
  }
  ...
  @Event() mfpOpen: EventEmitter<{ open: boolean }>;
  ...
  render() {
    const style = {
      ...(this.panelHeight && { '--mfp-panel--height': this.panelHeight }),
    };

    return (
      <div class="mfp-dropdown" part="base">
        {/* TRIGGER CONTAINER */}
        <div
          class="mfp-dropdown__trigger block"

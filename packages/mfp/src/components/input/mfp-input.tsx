...
import { TInputType, TInputValidation, TInputValue } from './mfp-input.types';
...
@Component({
  tag: 'mfp-input',
  styleUrl: './scss/mfp-input.scss',
  formAssociated: true,
  shadow: {
    delegatesFocus: true,
  },
})
export class MfpInput {
  // Own Properties
  // ====================
  ...
  private debounceMfpInput: TDebounce<void>;
  private fallbackInputId = 'input';

  // Reference to host HTML element
  // ===================================

  @Element() el!: HTMLMfpInputElement;
  ...
  @Event() mfpBlur!: EventEmitter<HTMLMfpInputElement>;

  /**
   * Callback handler emitted when the input value has changed and the input loses focus.
   * This handler is called whenever the user finishes typing or pasting text into the input field and then clicks outside of the input field.
   */
  @Event() mfpChange!: EventEmitter<{ value: string | number | string[]; el: HTMLMfpInputElement }>;

  /** Callback handler emitted when the input value has been cleared */
  @Event() mfpClear!: EventEmitter<HTMLMfpInputElement>;

  /** Callback handler emitted when the input has received focus */
  @Event() mfpFocus!: EventEmitter<HTMLMfpInputElement>;
   ...
   */
  @Event() mfpInput!: EventEmitter<{ value: string | number | string[]; el: HTMLMfpInputElement }>;
  ...
  private handleInput = (ev: Event) => {
    ...
    this.debounceMfpInput?.cancel();

    if (!isHTMLElement(ev.target, 'input')) return;
    this.value = this.type === 'number' ? Number(ev.target.value) : ev.target.value;
    this.internals.setFormValue(`${this.value}`);

    this.debounceMfpInput = debounce(() => {
      this.mfpInput.emit({ value: this.value, el: this.el });
    }, this.debounceTime);
    this.debounceMfpInput();
  };
  ...
  private handleClearClick = (ev: CustomEvent) => {
    ...
    const { mfpClear, mfpChange, mfpInput, el, inputElem } = this;
    // Emit events
    mfpClear.emit(el);
    mfpInput.emit({ value: this.value, el });
    mfpChange.emit({ value: this.value, el });
  ...
  render() {
    return (
      <div class="mfp-input" part="base">
        {/* Label */}
        <label
          class={{ 'mfp-input--label': true, '!hidden': !this.hasLabel }}
        ...
        >
          {/* Prefix */}
          <span
            class={{ 'mfp-input--control__prefix': true, '!hidden': !this.hasPrefix }}
          ...
          <input
            ...
            class="mfp-input--control__input"
          ...
          {this.hasValue && !this.disabled && !this.disableClear && (
            ...
            <mfp-button
              class="mfp-input--control__clear ms-[--mfp-input--gap] hidden"
              appearance="text"
          ...
          {/* Suffix */}
          <span
            class={{ 'mfp-input--control__suffix': true, '!hidden': !this.hasSuffix }}
        ...
        <div
          class={{
            [`mfp-input--helper-text validation-${this.validationStatus}`]: true,

...
import { DATE_PICKER_TYPE, DaysOfWeek, TDatePickerType } from './mfp-date-picker.types';
...
@Component({
  tag: 'mfp-date-picker',
  styleUrl: './scss/mfp-date-picker.scss',
  shadow: {
    delegatesFocus: true,
  },
})
export class MfpDatePicker {
  // Own Properties
  // ====================
  ...
  @Element() el!: HTMLMfpDatePickerElement;
  ...
  @Event() mfpBlur!: EventEmitter<HTMLMfpDatePickerElement>;

  /**
   * Callback handler emitted when the input value has changed and the input loses focus.
   * This handler is called whenever the user finishes typing or pasting text into the input field and then clicks outside of the input field.
   */
  @Event() mfpChange!: EventEmitter<{ value: string; el: HTMLMfpDatePickerElement }>;

  /** Callback handler emitted when the input value has been cleared */
  @Event() mfpClear!: EventEmitter<HTMLMfpDatePickerElement>;

  /** Callback handler emitted when the input has received focus */
  @Event() mfpFocus!: EventEmitter<HTMLMfpDatePickerElement>;
  ...
  private handleChange = (ev: Event) => {
    ...
    if (!isNaN(dateValue.getTime())) {
      ...
      this.displayDate = this.formatDisplayValue(this.value);
      this.mfpChange.emit({ value: this.value, el: this.el });
    }
  };
  ...
  render() {
    ...
    const labelId = `mfp-date-picker__label-${this.name || this.fallbackInputId}`;

    return (
      <div class="mfp-date-picker" part="base">
        {/* Label */}
        <label
          id={labelId}
          class={{ 'mfp-date-picker__label': true, '!hidden': !this.hasLabel }}
        ...
        <mfp-dropdown
          class="mfp-date-picker__dropdown is-full [&::part(panel)]:p-m [&::part(panel)]:is-auto"
        ...
        >
          ...
          >
            {/* Prefix */}
            <span
              class={{ 'mfp-date-picker__control--prefix': true, '!hidden': !this.hasPrefix }}
            ...
            <input
              ...
              class="mfp-date-picker__control--input"
              autoComplete="off"
              autoCapitalize="off"
            ...
            {this.hasValue && !this.disabled && !this.disableClear && (
              ...
              <mfp-button
                class="mfp-date-picker__control--clear ms-[--mfp-date-picker--gap] hidden"
                appearance="text"
              ...
              >
                <slot name="clear-icon">
                  <mfp-icon weight="bold" name="x-circle" class="flex" />
                </slot>
              </mfp-button>
            )}
            {/* Suffix */}
            <span
              class="mfp-date-picker__control--suffix"
              ref={(spanElem: HTMLSpanElement) => (this.suffixElem = spanElem)}
              part="suffix"
            >
              <slot name="suffix" onSlotchange={this.handleSuffixSlotChange}>
                <mfp-icon name="calendar-blank" weight="bold" class="flex" />
              </slot>
            </span>
          </div>
          <CallyCalendar
          ...
          >
            <mfp-icon color="text--primary" slot="previous" name="caret-left" label="Previous" />
            <mfp-icon color="text--primary" slot="next" name="caret-right" label="Next" />

            <div class="flex flex-wrap justify-center gap-[--mfp-spacing-m]">{this.generateCalendarMonths()}</div>
          </CallyCalendar>
        </mfp-dropdown>
      </div>
    );

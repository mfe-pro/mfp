...
import { TIconWeight } from './mfp-icon.types';
...
@Component({
  assetsDirs: ['svg'],
  tag: 'mfp-icon',
  styleUrl: './scss/mfp-icon.scss',
  shadow: true,
})
export class MfpIcon {
  // Own Properties
  // ====================
  ...
  handleWeightChange() {
    ...
    console.warn(
      `❗️ [mfp-icon]: the 'weight' property is deprecated, you should add the weight to the icon name.\n
       For example, '<mfp-icon name="bell-fill"></mfp-icon>' instead of '<mfp-icon name="bell" weight="fill"></mfp-icon>'`,
    );
  ...
  render() {
    const styles = {
      ...(this.color && { '--mfp-icon--color': getColorCSSVariable(this.color) }),
      ...(this.size && { '--mfp-icon--size': `${this.size}px` }),
    };
    ...
    return (
      <Host style={styles}>
        <div
          ...
          class="flex text-[color:--mfp-icon--color] bs-[--mfp-icon--size] is-[--mfp-icon--size]"

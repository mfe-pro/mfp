...
const fetchSvg = async (url: string, sanitize: boolean): Promise<unknown> => {
  ...
  try {
    ...
    console.error(`[MfpIcon] Failed to fetch SVG from ${url}:`, error);
    iconContent.set(url, '');
  }
};
...
export const validateContent = (svgContent: string): string => {
  const svgTag = 'svg';
  const iconCssClass = 'mfp-icon__svg';
  ...
  if (svgElm && svgElm.nodeName.toLowerCase() === svgTag) {
    // keep other CSS classes (if there's any) and add the `mfp-icon__svg` CSS class

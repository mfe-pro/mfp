...
const waitForSvgLoad = async (elem: HTMLMfpIconElement) => {
...
describe('mfp-icon', () => {
  it('should render', async () => {
    const page = await newE2EPage({
      html: '<mfp-icon></mfp-icon>',
    });

    const element = await page.find('mfp-icon');
  ...
  it('should display icon', async () => {
    const page = await newE2EPage({
      html: '<mfp-icon name="pulse"></mfp-icon>',
    });

    await page.$eval('mfp-icon', waitForSvgLoad);

    const element = await page.find('mfp-icon >>> [part="svg"]');
  ...
  it('should handle `name` property change', async () => {
    ...
    await setProperties(page, 'mfp-icon', { name: 'check' });
    await page.$eval('mfp-icon', waitForSvgLoad);
  ...
  it('should respect design style', async () => {
    ...
    const style = await computedStyle(page, 'mfp-icon >>> [part="base"]', ['height']);
  ...
  it('should change size', async () => {
    const page = await newE2EPage({
      html: '<mfp-icon size="30"></mfp-icon>',
    });

...
import { EMPTY_STATE_SIZE, SIZE_TO_VALUE_MAP, TEmptyStateSize } from './mfp-empty-state.types';
 ...
 * @part icon - The `<mfp-icon>` element used to render a predefined icon size based on the empty state size (small, medium, large)
...
@Component({
  tag: 'mfp-empty-state',
  styleUrl: './scss/mfp-empty-state.scss',
  shadow: true,
})
export class MfpEmptyState {
  // Own Properties
  // ====================
  ...
  @Element() el!: HTMLMfpEmptyStateElement;
  ...
  render() {
    return (
      <div class="inline-flex flex-col items-center" part="wrapper">
        ...
        >
          <slot name="thumbnail">
            <mfp-icon size={this.iconSize} name="database" part="icon" exportparts="base,svg" />
          </slot>
        </div>
        <div
          class={{

...
import { ALERT_TYPE, TAlertBorderRadius, TAlertType } from './mfp-alert.types';
 ...
 * @part btn-close - The `mfp-button` used to close the alert
 ...
 * @part icon - The `<mfp-icon>` element used to render a predefined icon based on the alert type (info, success, warning, error, default)
...
@Component({
  tag: 'mfp-alert',
  styleUrl: './scss/mfp-alert.scss',
  shadow: true,
})
export class MfpAlert {
  // Own Properties
  // ====================
  ...
  @Element() el!: HTMLMfpAlertElement;
  ...
  @Event() mfpAfterShow!: EventEmitter;

  /** Callback handler to be called after the alert has been hidden */
  @Event() mfpAfterHide!: EventEmitter;
  ...
  private handleTransitionEnd = () => {
    if (this.open) {
      this.mfpAfterShow.emit();
      return;
    }

    this.mfpAfterHide.emit();
  };
  ...
  render() {
    const style = {
      ...(this.border && { '--mfp-alert--border-radius': `var(--mfp-radius--${this.border})` }),
    };
    ...
    return (
      ...
      >
        <div
          class={{
            [`mfp-alert mfp-alert__${this.type}`]: true,
        ...
        >
          ...
          {!this.disableClose && (
            <mfp-button
              class="mfp-alert__close absolute end-5 focus-visible:focus [&::part(label)]:inline-flex"
              appearance="text"
              size="small"
          ...
          <div
            class={{
              [`mfp-alert__icon--${this.type} me-s flex text-left align-top`]: true,
              '!hidden': this.hideIcon,
            }}
            part="icon-outline"
          >
            <slot name="icon">
              {this.type !== 'default' && <mfp-icon name={this.iconName} part="icon" exportparts="base,svg" />}
            </slot>
          </div>
          {/* MAIN */}
          <div class="flex flex-col items-start gap-[--mfp-alert--content-footer-gap]" part="main">
            <div class="flex flex-col gap-[--mfp-alert--title-body-gap]" part="content">
              {/* TITLE */}
              <div
                class={{

...
import { TSliderType, TSliderValue } from './mfp-slider.types';
...
@Component({
  tag: 'mfp-slider',
  styleUrl: './scss/mfp-slider.scss',
  shadow: {
    delegatesFocus: true,
  },
})
export class MfpSlider {
  // Own Properties
  // ====================

  private inputMinElem: HTMLInputElement;
  private inputMaxElem: HTMLInputElement;
  private minTooltipElem: HTMLMfpTooltipElement;
  private maxTooltipElem: HTMLMfpTooltipElement;
  private progressElem: HTMLSpanElement;
  private trackElem: HTMLSpanElement;
  private debounceMfpChange: TDebounce<void>;

  // Reference to host HTML element
  // ===================================

  @Element() el!: HTMLMfpSliderElement;
  ...
  @Event() mfpChange: EventEmitter<{ value: Exclude<TSliderValue, string>; el: HTMLMfpSliderElement }>;

  /** Handler to be called when the slider loses focus */
  @Event() mfpBlur: EventEmitter<HTMLMfpSliderElement>;

  /** Handler to be called when the slider gets focused */
  @Event() mfpFocus: EventEmitter<HTMLMfpSliderElement>;
  ...
  private handleInputChange = (type: 'min' | 'max', event: InputEvent) => {
    ...
    // Sync the prop value.
    // This will trigger the `@Watch('value')` method and emit the `mfpChange` event.
  ...
  private calculateThumbPosition = (value: number): number => {
    ...
    const inputThumbSize = parseInt(getComputedStyle(this.el).getPropertyValue('--mfp-slider--thumb-size'), 10) + 4;
  ...
  private emitMfpChange = () => {
    this.debounceMfpChange?.cancel();

    const value: Exclude<TSliderValue, string> = this.isRangeType ? [this.minValue, this.maxValue] : this.minValue;
    this.debounceMfpChange = debounce(() => this.mfpChange.emit({ value, el: this.el }), this.debounceTime);

    this.debounceMfpChange();
  };
  ...
  private renderTooltip = (
    ...
    refCallback: (elem: HTMLMfpTooltipElement) => void,
  ): HTMLMfpTooltipElement => (
    <mfp-tooltip
      class={{
    ...
    return (
      ...
      >
        ...
        <div class="relative is-full" part="container">
          {/* TRACK AREA */}
          <span
            class="absolute start-0 -translate-y-1/2 rounded-xs bg-[--mfp-slider--trackarea-color] bs-1 is-full inset-bs-[50%]"
            ref={(elem) => (this.trackElem = elem)}
            part="track-area"
          />
          {/* PROGRESS AREA */}
          <span
            class="absolute -translate-y-1/2 rounded-xs bg-[--mfp-slider--progress-color] bs-1 is-[50%] inset-bs-[50%]"

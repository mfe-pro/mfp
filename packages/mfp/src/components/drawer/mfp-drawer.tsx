...
import { DRAWER_PLACEMENT, DRAWER_POSITIONS, TDrawerPlacement, TDrawerPosition } from './mfp-drawer.types';
 ...
 * @part button-close - The MfpButton that closes the drawer
...
@Component({
  tag: 'mfp-drawer',
  styleUrl: './scss/mfp-drawer.scss',
  shadow: true,
})
export class MfpDrawer {
  // Own Properties
  // ====================

  private footerElem: HTMLElement;
  private drawerElem: HTMLDivElement;
  private OPEN_CSS_CLASS = 'mfp-drawer--open';

  // Reference to host HTML element
  // ===================================

  @Element() el!: HTMLMfpDrawerElement;
  ...
  handlePlacementChange() {
    if (!isNil(this.placement)) {
      console.warn(
        `❗️ [mfp-drawer]: the 'placement' prop is deprecated and it will be removed in the future. Please use 'position' instead.`,
      );
    }
    ...
    return (
      <Host>
        ...
        {this.enableBackdrop && (
          <div
            class={{
              'fixed inset-0 block bg-[--mfp-drawer--backgroundBackdrop] opacity-0 transition-opacity duration-300':
                true,
        ...
        <div
          class={{
            [`mfp-drawer transition-all duration-300 ease-in-out ${this.position || this.placement}`]: true,
            '-start-[--mfp-drawer--width]': this.isPositionStart,
            '-end-[--mfp-drawer--width]': this.isPositionEnd,
            'start-0': this.open && this.isPositionStart,
            'end-0': this.open && this.isPositionEnd,
          }}
          ref={(div) => (this.drawerElem = div)}
          aria-hidden={!this.open ? 'true' : 'false'}
          aria-labelledby="mfp-drawer__title"
          aria-modal="true"
          hidden={!this.open}
          part="panel"
          role="dialog"
        >
            ...
            <div class="flex" part="button-close">
              ...
              >
                <slot name="button-close">
                  <mfp-icon name="x-bold" role="img" title="Close" />
                </slot>
              </mfp-button>
            </div>
          </header>
          ...
          >
            <slot name="footer-divider">
              <mfp-divider class="mb-m block" dashed />
            </slot>

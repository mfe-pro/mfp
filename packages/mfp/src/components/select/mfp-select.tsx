...
import { TInputValidation } from '../input/mfp-input.types';
 ...
 * @part tags - The tags container of the MfpTags for multiple selection.
 * @part tag - The tag container of the MfpTag for multiple selection.
 * @part tag__base - The base wrapper of the MfpTag for multiple selection.
 * @part tag__prefix - The prefix slot container of the MfpTag for multiple selection.
 * @part tag__text - The text slot container of the MfpTag for multiple selection.
 * @part tag__btn-close - The close button of the MfpTag for multiple selection.
 * @part option-list - The option list container.
 */
@Component({
  tag: 'mfp-select',
  styleUrl: './scss/mfp-select.scss',
  formAssociated: true,
  shadow: {
    delegatesFocus: true,
  },
})
export class MfpSelect {
  // Own Properties
  // ====================
  ...
  @Element() el!: HTMLMfpSelectElement;
  ...
  @State() selectedOptions: HTMLMfpOptionElement[] = [];
  ...
  /**
   * The amount of time, in milliseconds, to wait before emitting the `mfpInput` event after the input value changes.
  ...
  @Event() mfpBlur!: EventEmitter<HTMLMfpSelectElement>;

  /** Callback handler emitted when the selected value has been cleared */
  @Event() mfpClear!: EventEmitter<HTMLMfpSelectElement>;

  /** Callback handler emitted when the Select input has received focus */
  @Event() mfpFocus!: EventEmitter<HTMLMfpSelectElement>;

  /** Callback handler emitted when the selected value has changed */
  @Event() mfpSelect!: EventEmitter<{ value: string | number | string[]; item: HTMLMfpOptionElement }>;
  ...
  stopOptionFocusBlurPropagation(ev: CustomEvent) {
    // Stop propagation of focus and blur events coming from the `mfp-option` elements
    if (isHTMLElement(ev.target, 'mfp-select')) return;
  ...
  async clear(): Promise<void> {
    ...
    const { multiple, inputElem, mfpClear, el } = this;
  ...
  private handleSelect = (ev: CustomEvent<{ value: TSelectValue; item: HTMLMfpOptionElement }>) => {
    if (this.disabled) return;
    ...
    if (this.multiple) {
      ...
      this.mfpSelect.emit({ value: this.value, item });
    } else {
      this.value = value;
    }
  ...
  private handleMultipleSelection = (item: HTMLMfpOptionElement) => {
  ...
  private handleSearchFilter = (ev: Event) => {
    ...
    if (!isDefined(query)) {
      ...
      this.debounceQuery = debounce(() => {
        this.options.forEach((item: HTMLMfpOptionElement) => {
  ...
  private handleTagRemove = (item: HTMLMfpOptionElement) => {
    if (this.disabled) return;
  ...
  private resetOptionsVisibility = () => {
    this.options.forEach((item: HTMLMfpOptionElement) => (item.hidden = false));
  };
  ...
  private syncItemsFromValue = () => {
    ...
    // Sync selected state of the MfpOption elements
  ...
  /**
   * Syncs the selected state of the MfpOption elements which value is included in the `value` property.
  ...
  private syncSelectedOptionsState = () => {
    ...
    options.forEach((option: HTMLMfpOptionElement) => {
  ...
  private getOptionLabel = (item: HTMLMfpOptionElement) => {
    if (!item) return;
    return item.innerText.trim() ?? '';
  };

  private get options() {
    return Array.from(this.el.querySelectorAll('mfp-option'));
  }
      ...
      if (index < this.maxTagsVisible || this.maxTagsVisible < 0) {
        return (
          <mfp-tag
            ...
            variant="filled"
            onMfpClose={(event) => {
              // NOTE: prevents triggering mfpClose on parent
  ...
  render() {
    const labelId = `mfp-select__label-${this.name || this.fallbackInputId}`;

    return (
      <div class="mfp-select" part="base">
        {/* Label */}
        <label
          id={labelId}
          class={{ 'mfp-select__label': true, '!hidden': !this.hasLabel }}
        ...
        {/* Select dropdown */}
        <mfp-dropdown
          class="mfp-select__dropdown w-full"
        ...
        >
          ...
          >
            {/* Prefix */}
            <span
              class={{ 'mfp-select__control--prefix': true, '!hidden': !this.hasPrefix }}
            ...
            <div class="flex flex-1 overflow-x-auto" part="input-outline">
              {/* Display selected values using MfpTags for multiple selection */}
              {this.multiple && (
                <span class="mfp-select__tags" part="tags">
              ...
              <input
                ...
                class="mfp-select__control--input flex-grow is-full"
                autoComplete="off"
                autoCapitalize="off"
                autoFocus={this.autofocus}
                aria-disabled={this.disabled ? 'true' : 'false'}
                aria-controls={`mfp-options-${this.name}`}
            ...
            {this.hasClearIcon && (
              ...
              <mfp-button
                class="mfp-select__control--clear ms-[--mfp-select--gap]"
                appearance="text"
                aria-label={this.clearButtonLabel}
                size="small"
                onMfpClick={this.handleClearClick}
                part="clear-btn"
                exportparts="button"
                tabIndex={-1}
              >
                <slot name="clear-icon">
                  <mfp-icon name="x-circle" class="flex" />
                </slot>
              </mfp-button>
            )}
            {/* Suffix */}
            <span
              class={{ 'mfp-select__control--suffix': true, 'rotate-180': this.open, 'rotate-0': !this.open }}
              ref={(spanElem: HTMLSpanElement) => (this.suffixElem = spanElem)}
              part="suffix"
            >
              <slot name="suffix" onSlotchange={this.handleSuffixSlotChange}>
                <mfp-icon name="caret-down" class="flex" />
              </slot>
            </span>
          </div>
          <mfp-option-list
            id={`mfp-options-${this.name}`}
            onMfpSelect={this.handleSelect}
            aria-expanded={this.open ? 'true' : 'false'}
        ...
        <div
          class={{
            [`mfp-select__helper-text validation-${this.validationStatus}`]: true,

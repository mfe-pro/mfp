...
| ----------- | -------------------------------------------------- | ------------------------------------ |
| `mfpBlur`   | Handler to be called when the switch loses focus   | `CustomEvent<HTMLMfpSwitchElement>`  |
| `mfpChange` | Handler to be called when the switch state changes | `CustomEvent<{ checked: boolean; }>` |
| `mfpFocus`  | Handler to be called when the switch gets focus    | `CustomEvent<HTMLMfpSwitchElement>`  |
...
| `"icon-off"` | HTMLMfpIcon `<pk-icon>` element used as the `OFF` mark inner label |
| `"icon-on"`  | HTMLMfpIcon `<pk-icon>` element used as the `ON` mark inner label  |
...
graph TD;
  mfp-switch --> mfp-icon
  style mfp-switch fill:#f9f,stroke:#333,stroke-width:4px
```

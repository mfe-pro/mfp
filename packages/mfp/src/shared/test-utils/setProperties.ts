...
export type BqHTMLTagNameMap = keyof {
  [TAG in keyof HTMLElementTagNameMap as TAG extends `mfp-${string}` ? TAG : never]: TAG extends `mfp-${string}`
    ? TAG
    : never;
};

/**
 * Enhances page to set and retrieve properties based on attributes parameter. It typed against mfp web components
